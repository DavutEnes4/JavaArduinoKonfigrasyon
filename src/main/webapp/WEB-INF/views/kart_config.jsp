...
<head>
    <meta charset="UTF-8">
    <title>${kart.adi} (#${kart.id}) <spring:message code="config.title"/></title>
...
<div class="container py-5">
    ...
    <form id="pinConfigForm" action="${pageContext.request.contextPath}/kart/${kart.id}/config/save" method="post" autocomplete="off">
        ...
        <div class="form-floating mb-4" style="max-width:400px;">
            <input type="text" class="form-control" name="isim" id="isimInput" placeholder=" " required>
            <label for="isimInput"><spring:message code="config.name"/></label>
        </div>
                ...
                <tr>
                    ...
                    <td>
                        <select class="form-select" name="arduinoPinler">
                            ...
                            <c:forEach var="pinStr" items="${fn:split(arduinoPinListesi, ',')}">
                                <c:set var="pinArr" value="${fn:split(pinStr, ':')}" />
                                <option value="${pinArr[1]}">${pinArr[0]}</option>
                            </c:forEach>
                        </select>
                    </td>
                </tr>
        ...
        <button class="btn btn-success"><spring:message code="save"/></button>
    </form>

    <div class="mb-3">
        <label class="form-label"><spring:message code="selected.pins"/></label>
        <pre id="pinPreview" class="bg-light p-2 border"></pre>
    </div>
</div>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
<script>
    // Canlı önizleme (value'lar)
    function updatePreview() {
        let selectedPins = [];
        document.querySelectorAll('select[name="arduinoPinler"]').forEach(sel => {
            if (sel.value) selectedPins.push(sel.value);
        });
        document.getElementById('pinPreview').innerText = selectedPins.join(',');
    }
    ...
    document.getElementById('pinConfigForm').addEventListener('submit', function(e) {
        e.preventDefault();
        const isim = document.getElementById('isimInput').value;
        const pinValues = document.getElementById('pinPreview').innerText;
        const body = 'isim=' + isim + '&pinValues=' + pinValues;
        fetch(this.action, {
            method: 'POST',
            headers: {
